...
# export TORCHX_INTERACTIVE=1 \
# eval_on_cluster_name=gt_copycat \
# models=gt_copycat \
# dp=8 \
# tp=1
...
# bash scripts/eval_locally/eval_suite-template-gt_copycat.sh
# python analysis/gather_metrics.py /home/vecglypher/mnt/workspace/svg_glyph_llm/outputs/gt_copycat
set -e
...
STORAGE_MODEL_PATH="specific_baselines"
MODEL_PATH="saves/specific_baselines"
OUTPUT_DIR="/mnt/workspace/svg_glyph_llm/outputs/specific_baselines-re_org/debug"
DATA="/mnt/workspace/svg_glyph_llm/data/processed/filtered_sft/250903-alphanumeric/ood_font_family_decon"
DRY_RUN=false
MAX_TOKENS=3000
TEMPERATURE=0.0
TOP_P=1.0
...
#     python src/serve/api_infer_gt_copycat.py \
#         --data "${data}" \
...
if [[ -f "${done_ocr_eval_flag}" ]]; then
    ...
    python -m src.eval.score_ocr_eval "${output_dir}"/ocr_infer "${output_dir}"/results_ocr_eval-no_use_case --no_use_case --skip_number
    python -m src.eval.score_ocr_eval "${output_dir}"/ocr_infer "${output_dir}"/results_ocr_eval-use_case --use_case --skip_number
...
if [[ -f "${done_img_eval_flag}" ]]; then
    ...
    storage_vgg_model_path="workspace/hf_downloads/eval_ckpts/vgg16-397923af.pth"
    ...
    HF_DINO_MODEL_PATH=${STORAGE_MOUNT_DIR}/workspace/hf_downloads/facebook/dinov2-base \
    HF_CLIP_MODEL_PATH=${STORAGE_MOUNT_DIR}/workspace/hf_downloads/openai/clip-vit-base-patch32 \
    TORCH_HUB_CKPT_DIR=${STORAGE_MOUNT_DIR}/workspace/hf_downloads/eval_ckpts \
    OPENAI_CLIP_CACHE_DIR=${STORAGE_MOUNT_DIR}/workspace/hf_downloads/eval_ckpts \

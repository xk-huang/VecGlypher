...
class FIDCalculator(BaseMetric):
    def __init__(
        ...
        if self.model_name == "ViT-B/32":
            self.dims = 512
            download_root = None
            if os.environ.get("OPENAI_CLIP_CACHE_DIR", None) is not None:
                download_root = os.environ["OPENAI_CLIP_CACHE_DIR"]
                print(f"Loading CLIP models from {download_root}")
            model, preprocess = clip.load("ViT-B/32", download_root=download_root)
    ...
    def get_activations(self, images, batch_size=256, num_workers=16):
    ...
    def calculate_score(self, batch):
        print("calculating activation statistics for gt")
        m1, s1 = self.calculate_activation_statistics(batch["gt_im"])
        print("calculating activation statistics for gen")

...
class CountTokenLength(BaseMetric):
    ...
    def calculate_score(self, batch, update=None):
        ...
        values = []
        for gt_svg, gen_svg in tqdm(
            zip(gt_svgs, gen_svgs), total=len(gt_svgs), desc="Processing SVGs"
        ):
            gt_tokens, gen_tokens, diff = self.calculate_token_length(
                gt_svg=gt_svg, gen_svg=gen_svg
            )
            self.meter_gt_tokens.update(gt_tokens, 1)
            self.meter_gen_tokens.update(gen_tokens, 1)
            self.meter_diff.update(diff, 1)
            values.append(
                {"gt_tokens": gt_tokens, "gen_tokens": gen_tokens, "diff": diff}
            )
        avg_score = {
